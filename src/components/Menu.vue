...
<template>
    <div class="bg-indigo-950 text-gray-200 w-full inset-0 sticky z-20 ">
		...
		<header class="mx-[7%] p-4 sm:p-2">
			<div class="flex justify-between items-center">
				<nav class="hidden lg:flex space-x-6 py-2">
					<a v-for="(item,idx) in content.links" :key="idx" :href="item.url" class="hover:text-indigo-400"  >{{ item.text }}</a>
				</nav>
		
				<button id="menu-btn" @click="showMenuMobil = !showMenuMobil" class="lg:hidden text-white focus:outline-none">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
						<path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
					</svg>                      
				</button>
			
				<nav id="menu" class="hidden lg:flex space-x-3 ">
			...
			<div id="mobile-menu" class="lg:hidden flex-col space-y-1 mt-5" :class="showMenuMobil ? 'flex' : 'hidden' " >
